...
def main():
	...
	parser.add_option("-o","--output",action="store",type="string",dest="output_file", help="Prefix of output files. Will generate two intermediate files: \"prefix.SNP.tsv\", \"prefix.PI.filtered.tsv\"")
	...
	if total_hom >= 10:
		...
		print >>sys.stderr, "\nOverall contamination level of %s is %.3f"  % (options.bam_file.replace('.bam',''), pi_final)
		...
		print >>sys.stderr, "\nOverall contamination level of %s is %s"  % (options.bam_file.replace('.bam',''), 'unknown')
